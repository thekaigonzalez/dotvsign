...
class VSign:
    ...
    def get_web_latest_version(self):
        return json.loads(requests.get(self.url).text)["tag_name"] #TODO: allow multiple formats
    ...
    def compare_vsign(self):
        if (self.parser == "v1"):
            if (parse_v1(self.get_local_version()) < parse_v1(self.get_web_latest_version())):
                print("This version is not up to date!")
                cprint("you can get the latest versions here: " + self.dlurl, "white", attrs=["dark"])
        elif (self.parser == "v2"):
            if (parse_v2(self.get_local_version()) != parse_v2(self.get_web_latest_version())):
                print("This version is not up to date!")
                cprint("you can get the latest versions here: " + self.dlurl, "white", attrs=["dark"])
        elif self.parser == "v3":
            if (parse_v3(self.get_local_version()) < parse_v3(self.get_web_latest_version())):
                print("This version is not up to date!")
                cprint("you can get the latest versions here: " + self.dlurl, "white", attrs=["dark"])
